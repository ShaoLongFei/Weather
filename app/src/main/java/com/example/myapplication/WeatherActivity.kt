package com.example.xiaoguoweather
...
import com.example.xiaoguoweather.utils.HttpUtil
import com.example.xiaoguoweather.utils.Utility
    ...
    fun requestWeather(countyName: String) {
        ...
        HttpUtil.sendOkHttpRequest(url, object : Callback {
            override fun onFailure(call: Call?, e: IOException?) {
                e!!.printStackTrace()
                runOnUiThread {
                    Toast.makeText(this@WeatherActivity, "获取天气信息失败",
            ...
            override fun onResponse(call: Call?, response: Response?) {
                ...
                runOnUiThread {
                    if (weather != null && "ok" == weather.status) {
                        val edit = PreferenceManager.getDefaultSharedPreferences(this@WeatherActivity)
                                .edit()
